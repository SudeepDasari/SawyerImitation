...
class ImitationLearningModel:
    ...
    def build(self):
        with slim.arg_scope([slim.layers.conv2d, slim.layers.fully_connected, tf_layers.layer_norm]):
            ...
            fc_layer1 = slim.layers.fully_connected(fp_flat, 100, scope='fc1')
            fc_layer2 = slim.layers.fully_connected(fc_layer1, 75, scope='fc2')

            self.predicted_eeps = slim.layers.fully_connected(fc_layer2, 7, scope='predicted_ee_point',
                                                          activation_fn=None)  # dim of eeps: 3

            eeps_fed = tf.concat([self.predicted_eeps, self.robot_configs], 1)
            fc_layer3 = slim.layers.fully_connected(eeps_fed, 25, scope='fc3')
            self.predicted_actions = slim.layers.fully_connected(fc_layer3, 7, scope = 'predicted_dff', activation_fn=None)
