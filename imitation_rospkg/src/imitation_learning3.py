...
class ImitationLearningModel:
    ...
    def build(self):
        with slim.arg_scope([slim.layers.conv2d, slim.layers.fully_connected, tf_layers.layer_norm]):
            ...
            fc_layer1 = slim.layers.fully_connected(conv_out, 50, scope='fc1')

            self.predicted_eeps = slim.layers.fully_connected(fc_layer1, 3, scope='predicted_eeps',
                                                              activation_fn=None)  # dim of eeps: 3

            conv_out_eeps = tf.concat([fp_flat,
                                  self.robot_configs,  # dim of angles: 7, dim of eeps: 3
                                  self.predicted_eeps],
                                 1)

            fc_layer2 = slim.layers.fully_connected(conv_out_eeps, 100, scope='fc2')
