...
def setup_MAML_predictor(meta_path, norm_path, crop_h_min = 0, crop_h_max = -150, crop_w_min = 190, crop_w_max = -235):
    ...
    def predictor_func(image_a, record_state, record_action, imageb, robot_state):
        img_a, img_b, state_a, state_b, action_a = load_video_ref(image_a, imageb, record_state, record_action,
                                                                  robot_state,
                                                                  scale, bias)

        pred_actions, f_ee = sess.run([output, output_f_ee],
                                      feed_dict={obsa: img_a, obsb: img_b, statea: state_a, stateb: state_b,
                                                 actiona: action_a})
